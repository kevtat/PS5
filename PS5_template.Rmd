...
```{r}
#Asia
df %>% filter(is.na(co2_PC)) %>%
  filter(region == "Asia") %>%
  filter(time == "2019") %>%
  group_by(name) %>%
  summarise(avgco2pc = mean(co2_PC)) %>%
  arrange(avgco2pc) %>%
  tail(3) %>%
  pull(name)
#Africa
...
```{r}
df_1960_2019 <- df %>% 
  filter(time %in% c("1960", "2019"), !is.na(lifeExpectancy), !is.na(iso3))

  dfavgLE <- df_1960_2019 %>% 
  group_by(time, region) %>% 
  summarize(AverageLE = mean(lifeExpectancy), .groups = 'drop')
  
  dfLEgrowth <- dfavgLE %>% 
  group_by(region) %>% 
  mutate(growth = (AverageLE / lag(AverageLE)) - 1) %>% 
  filter(time == "2019") %>% 
  arrange(desc(growth))
  dfLEgrowth
```
...
sum <- df %>%
  filter(time == "1960") %>%
  summarize(sum_DPC_1960 = sum(GDP_PC))

sum2 <- df %>%
  filter(time == "2019") %>%
  summarize(sum_DPC_2019 = sum(GDP_PC))
...
hist(filter_data_2019, col = "green",xlim = c(0, 40000), ylim = c(0,100), main = "Histogram of GDP per capita for 1960 and 2019", xlab = "GDP per capita")
hist(filter_data_1960, col = "blue", add = TRUE)
legend("topright", c("1960", "2019"), fill = c("blue", "green"))
